...
function loadMoneyHelpContent( event, htmlFileName )
{
	...
	xhr.send();
			...
			var currentLanguage = 'ta'; // default Tamil
			var englishData = {}; // Store API/English values
			var tamilData = {};
			function toggleLanguage()
			{
				...
				else
				{
					...
					// Show Tamil values asynchronously
					(async () => {
						document.getElementById('title').innerText = "உங்கள் இருப்பிடம்";
    ...
    navigator.geolocation.getCurrentPosition(
        async ( position ) => {
            ...
            try
            {
                ...
                englishData.district = addr.state_district || addr.county || "N/A";
                englishData.state = addr.state || "N/A";
                englishData.country = addr.country || "N/A";
                englishData.continent = getContinent((addr.country_code||"").toUpperCase());

                // Fill table asynchronously in Tamil
                (async () => {
                    document.getElementById("district").innerText = await toTamil(englishData.district);
                    document.getElementById("state").innerText = await toTamil(englishData.state);
                    document.getElementById("country").innerText = await toTamil(englishData.country);
